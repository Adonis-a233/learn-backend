# learn-backend 第一轮：Java 起步任务与赛事数据处理

> 先做一个小工具：输入命令，查出比赛结果。顺便让它遇到输错的命令、找不到的文件时，也能把事情交代清楚。

本轮主要考察 Java 基础语法，以及运用类、集合和文件读写完成小程序的能力，**不需要答辩**，通过提交的代码、测试和 README 检查完成情况。不要求复杂架构，也不用提前学习 Web 框架或数据库。

**测试和异常处理从本轮开始就是必做内容。** 每增加一个功能，都想一想：正常输入会得到什么，边界在哪里，失败时程序该怎么办，再用测试验证。能跑通一次只是开始，修改后仍然正确、出错时不悄悄给出错误结果，同样是项目的一部分。

## 开始前

- 推荐使用 **IntelliJ IDEA**、**JDK 25 及以上，优先当前最新正式版**，以及最新版 Maven 或 Gradle 管理依赖和测试。当前版本入口见[第 0 轮](0-开始之前.md)。
- 安装和学习资料见 [Java 推荐资料](0-推荐资料-java.md)。先完成三个小检查：创建项目、运行 `main` 方法、执行一个测试。
- 在 README 中记录实际 JDK、构建工具版本和运行命令。

## AI 学习方式

**本轮建议亲手编写主要代码。** 任务以熟悉语法、代码、调试和测试为主，先自己尝试实现，再请 AI 解释报错、给提示或评审思路。保留下面的辅助提示词，但尽量不要直接生成整套项目；学习目标见[第 0 轮：开始之前](0-开始之前.md)。

提问时带上当前日期、操作系统、JDK、IDE 和构建工具版本，以及已掌握的内容和卡住的现象。涉及版本、API 或配置时，要求提供官方来源并自行核验；保留真正解决问题的提问、来源和自己的结论即可。

- [提示词一：生成第一轮学习路线](prompts/round1/prompt1.md)
- [提示词二：理解 Java 基础与面向对象](prompts/round1/prompt2.md)
- [提示词三：文件、数据与命令解析](prompts/round1/prompt3.md)
- [提示词四：测试、异常与项目结构](prompts/round1/prompt4.md)
- [提示词五：排查第一轮具体问题](prompts/round1/prompt5.md)

## 会用到的知识

- 变量、基本类型、字符串、分支、循环、方法和数组；
- 类、对象、封装，接口、继承和多态的基本用途；不必为了用上所有语法而增加类；
- `List`、`Map`、泛型和排序；
- 命令行参数、路径、UTF-8 文本和文件读写；
- 异常处理、异常传播和资源关闭；
- Maven/Gradle、JUnit、断言和 Git 的基本使用。

按项目需要逐步学习即可。并发、复杂设计模式和性能优化不作为本轮基础要求。

## 项目：DWASearch

给跳水比赛做一个离线查询工具。使用题目提供的 **2026 World Aquatics Diving World Cup Super Final（北京）固定数据包**，查询选手信息和决赛结果，并写入文本文件。

数据位于 [`data/round-1-diving-2026/`](../data/round-1-diving-2026/)：`data.json` 包含选手和比赛结果，`manifest.json` 记录来源、抓取时间、版本与校验值。将数据包复制到自己的项目中，在 README 写明目录和版本。运行与测试只读取本地数据，不修改原始文件，也不依赖赛事官网。字段说明见 [数据包说明](../data/round-1-diving-2026/README.md)。

程序接收两个命令行参数，分别为指令文件和输出文件：

```bash
java -jar DWASearch.jar input.txt output.txt
```

### 1. 查询选手

输入 `players`，输出全部选手的全名、性别和国籍。按国籍升序、姓氏（Last Name）升序排列；固定数据已排序，可以直接沿用，并在 README 说明。

输出样例（节选）：

```text
Full Name:COLE Ellie
Gender:Female
Country:Australia
-----
Full Name:KEENEY Maddison
Gender:Female
Country:Australia
-----
```

### 2. 查询决赛结果

输入 `result <项目名称>`，按排名升序输出该项目的决赛结果。姓名对应 `fullName`，排名对应 `rank`，成绩按 `scores` 中的顺序连接，并在等号后输出 `totalPoints`；保持数据中的两位小数。若自行计算成绩，应考虑精度，不能直接依赖二进制浮点数。

支持以下八个项目：

```text
women 3m springboard
women 10m platform
women 3m synchronised
women 10m synchronised
men 3m springboard
men 10m platform
men 3m synchronised
men 10m synchronised
```

例如，`result women 3m springboard` 的输出如下（节选）：

```text
Full Name:CHEN Jia
Rank:1
Score:73.50 + 74.40 + 76.50 + 72.00 + 78.00 = 374.40
-----
Full Name:CHEN Yiwen
Rank:2
Score:69.00 + 72.00 + 76.50 + 79.05 + 76.50 = 373.05
-----
```

### 3. 多条命令与输出约定

指令文件可以包含多行，也可以重复查询：

```text
players
result women 3m springboard
result women 3m springboard
```

- 逐行按顺序处理，重复命令应再次输出结果；不要把重复命令去掉。
- 输入输出统一使用 UTF-8，输出换行使用 `\n`。
- 每条选手、比赛结果或错误信息后，都以 `-----` 单独一行分隔，最后一条也一样；命令之间不额外插入空行。
- 字段名、冒号和空格按样例输出，不增加首尾空格；样例中的节选不代表只需输出这些记录。
- 命令区分大小写，按文档中的小写形式匹配；项目名称中的空格不能随意增加。
- 空行可以选择忽略或作为非法命令处理，在 README 写明并用测试固定该行为；空文件应产生空输出。

### 4. 输错命令怎么办

无法识别的命令输出 `Error`。能识别为 `result`（后面是空格或行尾），但项目缺失、不在上述列表内，或名称后多了字符时，输出 `N/A`。每条错误后仍输出分隔线，继续处理后续命令。

| 输入 | 输出信息（下一行均为 `-----`） |
| --- | --- |
| `player`、`Players`、`players extra` | `Error` |
| `resultwomen 3m springboard` | `Error` |
| `result`、`result sss` | `N/A` |
| `result women 10m springboard` | `N/A` |
| `result women 3m springboard details` | `N/A` |
| `result men 10m     synchronised` | `N/A` |

### 5. 文件或数据出问题怎么办

输错一行命令可以继续处理，但输入文件打不开、赛事数据损坏时，程序已经无法可靠完成查询。至少处理以下情况：

- 命令行参数数量不对：提示正确用法；
- 输入文件或数据文件不存在、无法读取：说明哪个文件出了问题；
- 数据 JSON 无法解析或缺少查询所需字段：说明数据问题，不伪造空结果或默认成绩；
- 输出路径无法写入或写入失败：报告失败，不提示“查询成功”。

这些故障应向标准错误输出给出可理解的提示，并以非零退出码结束；不要把它们混成业务结果里的 `Error` 或 `N/A`。写入中途失败时可能留下部分输出，要明确提示输出可能不完整，本轮不要求实现原子写入。

合理关闭文件等资源，保留有助于定位问题的原因。不要用一个空的 `catch` 把问题藏起来，也不要捕获异常后继续输出看似正常的结果。测试坏数据时创建独立样本，不改题目原始数据。

## 测试：别只试“正常情况下”

使用 **JUnit** 编写可重复执行的自动化测试。写一个功能，就为它补上正常、边界和失败情况的检查，不要等全部完成后才补测试。

| 检查对象 | 至少验证什么 |
| --- | --- |
| 选手与成绩查询 | 返回的数据、顺序和成绩格式符合固定数据 |
| 命令处理 | 多行、重复命令、未知命令、缺失或错误项目名称 |
| 边界与格式 | 空文件、约定的空行行为、大小写、多余空格、分隔线和换行 |
| 错误后继续 | 一条非法命令后接一条合法命令，两条都按规则输出 |
| 数据读取 | JSON 损坏、缺少必要字段时明确失败 |
| 程序入口与文件 | 参数数量错误、文件不存在、输出不可写时有提示，并返回非零退出码 |

测试需要断言预期结果，不能只打印出来靠肉眼判断。不规定数量或覆盖率百分比，但关键行为和失败路径都应有证据。

核心逻辑可以直接传入数据测试，文件行为使用临时文件或目录验证，不依赖自己电脑上的固定路径。另保留一次真实命令行运行检查，确认打包后的程序也能读取输入并生成输出。

遇到 Bug 时，先保留能复现它的输入，修复后把它加入回归测试，确认同类问题不会悄悄回来。可参考：[单元测试和回归测试](https://www.cnblogs.com/xinz/archive/2011/11/20/2255830.html)。

## 代码组织与提交

把命令解析、查询和格式化整理成可以独立调用的方法或类，让测试不必每次启动整个命令行程序。文件读写与这些逻辑适当分开即可，不规定 `CoreModule` 等类名，也不需要复杂分层或设计模式。

使用 Maven/Gradle 的常规项目布局，保留含 `main` 方法的 `DWASearch.java` 作为入口。把程序打包成 jar，在 README 写明打包和运行命令；不要将 jar 等构建产物提交到仓库。

本轮由负责人统一收录到本仓库根目录的 [`work1/<GitHub ID>/`](../work1/README.md)，按当届公布的方式提交，无需另建独立项目仓库。个人目录包含源码、固定数据包、依赖与构建配置、自动化测试、README 和 `.gitignore`，能够独立运行。不提交密钥、个人敏感信息、IDE 缓存或编译产物。

README 简要说明：

1. 环境版本、数据目录与版本，以及构建、运行和测试命令；
2. 已完成功能、空行处理规则和已知限制；
3. 数据读取、命令处理、结果输出分别由哪些方法或类负责；
4. 测试覆盖哪些行为、实际执行结果，以及主要故障如何处理。

不要求长篇设计报告或 PPT，写到别人能够运行、检查和继续修改即可。

## 本轮完成检查

**本轮不答辩，以代码、自动化测试和 README 为准。** 提交前检查：

- 在说明的环境中，按 README 能构建、运行程序和执行测试，路径不依赖个人电脑；
- 两类查询、多命令和非法输入符合约定，相同输入与固定数据产生相同输出；
- 测试有明确断言，覆盖正常、边界与失败行为，并已实际执行；
- 文件和数据故障能被清楚报告，资源得到关闭，没有吞掉异常或伪造成功；
- 能通过代码和 README 看清基本处理过程，没有直接写死样例结果。

卡住时可以带着当前代码、预期结果、实际错误和尝试过程找我们聊，不用等到提交时再一起解决。

## Bonus

基础功能和测试完成后，可以选一个感兴趣的方向：

- 增加 CSV 等数据源，保留原有 JSON 查询能力；
- 提供表格化终端展示，同时保留规定的文件输出；
- 测量一组更大数据下的运行情况，尝试优化，并用原有测试确认结果没有改变。

Bonus 不抵消缺失的基础功能、测试或异常处理。先把小工具做踏实，再给它加点花样。
