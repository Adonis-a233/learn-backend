...
本轮须自行创建一个**公开 GitHub 仓库**，将本轮全部代码、README 和必要的运行说明提交并推送到该仓库；不能只在本地保存或只上传截图、可执行文件。仓库中不得包含密钥、个人敏感信息、IDE 缓存或编译产物。
...
2. 使用清晰的 commit message、`.gitignore` 和 README 维护本轮公开仓库，使其他人能根据说明运行和检查你的代码。
