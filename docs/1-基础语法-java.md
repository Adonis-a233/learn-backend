...
- 推荐使用 **IntelliJ IDEA**、**Java 21 或 Java 25**，以及 Maven 或 Gradle 管理依赖和测试。
- 安装和学习资料见 [Java 推荐资料](0-推荐资料-java.md)。先完成三个小检查：创建项目、运行 `main` 方法、执行一个测试。
- 在 README 中记录实际 JDK、构建工具版本和运行命令。

## AI 学习方式

可以让 AI 帮你拆分知识、解释错误、设计练习和评审思路，但不要提交自己无法解释、修改或调试的代码。
...
自行创建一个**公开 GitHub 仓库**，提交源码、固定数据包、依赖与构建配置、自动化测试、README 和 `.gitignore`，再将仓库链接交到负责人指定入口。不提交密钥、个人敏感信息、IDE 缓存或编译产物。
