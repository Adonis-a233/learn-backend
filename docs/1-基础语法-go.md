...
开始前先读[第 0 轮：AI 与学习方式](0-开始之前.md)。本轮建议亲手编写主要代码，练习语法、调试和测试；可以让 AI 解释错误或提供提示，先自己实现，再核对和改进。Go 与工具优先使用最新正式版，明显适配不足时再调整并说明。
...
本轮由负责人在本仓库根目录 [`work1/<GitHub ID>/`](../work1/README.md) 统一收录，按当届公布的方式提交完整代码、README、测试和必要的运行说明，无需另建独立项目仓库；不能只上传截图或可执行文件。不得包含密钥、个人敏感信息、IDE 缓存或编译产物。下面的 fork 练习用于学习 Git 协作，不替代本轮作业收录。
...
2. 使用清晰的 commit message、`.gitignore` 和 README 维护本轮作业，使其他人能根据说明运行和检查你的代码。
